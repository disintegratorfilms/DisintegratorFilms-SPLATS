const STORAGE_KEY = "the-king-is-dead-camera-states-v1";
...
const DEFAULT_STATES = [
  {
    ...
    infoBody:
      "Chris Bohn builds cinematic visual worlds that connect photography, 3D design, animation, splats, physical making, and real-time presentation into one dark, cohesive practice.",
    captionCamera: { x: 1.32, y: 0.16, depth: 2.7, scale: 0.58 },
    captionScale: 1.2,
    view: {
      position: [1.253625, 2.170307, -1.719658],
      target: [-0.505847, 1.507479, -0.372764],
      angles: [-16.653734, 127.434425, 0],
      distance: 2.312835,
      fov: 75,
      rotation: [-0.064127, 0.887167, 0.129849, 0.438134],
    },
  },
  {
    id: "state-1776640776844",
    label: "State 02",
    transition: {
      easing: "easeInOutCubic",
      duration: 1.2,
    ...
    eyebrow: "Practice",
    copy:
      "Cinematic environments, experimental visuals, and immersive digital worlds built for screen, installation, and space.",
    ...
    captionCamera: { x: 1.52, y: 0.38, depth: 3.3, scale: 0.62 },
    captionScale: 1.24,
    view: {
      position: [4.597307, 3.065146, 3.509954],
      target: [-0.505847, 1.507479, -0.372764],
      angles: [-13.653734, 52.734425, 0],
      distance: 6.598788,
      fov: 75,
      rotation: [-0.106503, 0.440974, 0.052793, 0.889614],
    },
  },
  {
    id: "state-1776640810076",
    label: "State 03",
    transition: {
      easing: "easeInOutCubic",
      duration: 1.2,
    ...
    captionCamera: { x: 1.72, y: 0.34, depth: 3.05, scale: 0.6 },
    captionScale: 1.18,
    view: {
      position: [-2.816186, 1.888364, -2.701873],
      target: [-0.244401, 1.096286, -0.453174],
      angles: [-13.053734, 228.834425, 0],
      distance: 3.506867,
      fov: 75,
      rotation: [0.046988, 0.904658, 0.103502, -0.410699],
    },
  },
  {
    id: "state-1776640836689",
    label: "State 04",
    transition: {
      easing: "easeInOutCubic",
      duration: 1.2,
    ...
    captionCamera: { x: 1.48, y: 0.22, depth: 3.2, scale: 0.62 },
    captionScale: 1.24,
    view: {
      position: [2.965406, 0.964382, 0.242925],
      target: [-0.465682, 1.117821, -0.121668],
      angles: [2.546266, 83.934425, 0],
      distance: 3.453815,
      fov: 75,
      rotation: [0.01652, 0.66854, -0.014858, 0.743344],
    },
  },
  {
    id: "state-1776640861391",
    label: "State 05",
    transition: {
      easing: "easeInOutCubic",
      duration: 1.2,
    ...
    eyebrow: "Contact",
    copy:
      "Open for commissions, collaborations, and visual projects that need a world that tells a story.",
    ...
    captionCamera: { x: 1.18, y: 0.12, depth: 3.55, scale: 0.66 },
    captionScale: 1.26,
    view: {
      position: [-1.286208, 5.570925, 0.348151],
      target: [-0.4138, 1.057594, 0.385716],
      angles: [-79.05, -92.465575, 0],
      distance: 4.597028,
      fov: 75,
      rotation: [0.440227, 0.557033, 0.459591, -0.533564],
    },
  },
];
...
function loadStates() {
  const preferStorage = new URL(window.location.href).searchParams.get("source") === "storage";
  if (!preferStorage) {
  ...
  try {
    ...
    return parsed.map((state, index) => {
      ...
      return {
        ...
        captionCamera: fallback.captionCamera,
    ...
    console.warn(error);
    if (preferStorage) {
      setStatus("warn", "Could not read the saved king-scene camera states.");
    }
    return structuredClone(DEFAULT_STATES);
  }
}
